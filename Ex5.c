    ...
    if (!orientado) {   //garante espelhamento 
        for (int i = 0; i < matricial->n; i++) {
            for (int j = i; j < matricial->n; j++) {
                if (i != j) {                          //evitar ligacoes proprias
                    matricial->matriz[i][j] = (rand() % 100 < probabilidade) ? (rand() % 9 + 1) : 0;//random de números entre 0 e 99 (resto da divisao por 100)
        ...
        for (int i = 0; i < matricial->n; i++) {
            for (int j = 0; j < matricial->n; j++) {
                if (i != j) {                          //evitar ligacoes proprias
                    matricial->matriz[i][j] = (rand() % 100 < probabilidade) ? 1  * (rand() % 9 + 1) : 0;//random de números entre 0 e 99 (resto da divisao por 100)
                } else {
                    matricial->matriz[i][j] = 0;       //falso para quando for a diagonal principal       
                }
            }
...
/*
Função para checar se todos os elementos da lista possuem o vértice inserido como vizinho.
...
bool conectado(Matriz* grafo, p_lista list, int vertice) {
    ...
    while (list != NULL) {
        ...
        if (!(grafo->matriz[vertice][list->item] || grafo->matriz[list->item][vertice]))
            return false;
...
void imprimir_resultado_arquivo(p_lista list) {
    ...
    printf("\nDigite o nome do arquivo texto a exportar os dados (adicionar .txt no fim):\n");
    scanf(" %s",buffer);
...
/*
Função que solicita ao usuário o preenchimento da probabilidade e da orientação via prompt
...
/*
Função para fazer um teste completo aleatório. Cria arquivos de entrada e saída usados.
...
/*
Procedimento que cria a interface básica para o usuário interagir com o programa.
...
int menu() {
    ...
    do {
        printf("\n\n+-------------------Menu---------------------+\n|1 - Inserir dados                           |\n|2 - Carregar dados                          |\n|3 - Criar caso de teste completo            |\n|4 - Inserir grafo manualmente               |\n|5 - Aleatorizar grafo                       |\n|6 - Procurar clique maximal de um vértice   |\n|7 - Salvar resultado em um arquivo          |\n|8 - Imprimir resultado na tela              |\n|0 - Sair                                    |\n+--------------------------------------------+\n");
        scanf(" %d", &opcao);
...
int main() {
    ...
    while (true) {
        ...
        case 4:
            if(qtd_vertices != -1) {
                if (grafo == NULL)
                    inicializar_matriz(qtd_vertices);
        ...
        case 6:
            if (grafo != NULL) {
                ...
                clique_maximal(grafo,vertice-1);
                contador_cliques++;
            } else {
